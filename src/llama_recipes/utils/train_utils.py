...
#                         break
#                     epoch_total_tokens += batch['input_ids'].numel()
...
#                                 'train/tokens_per_second': (epoch_total_tokens * torch.cuda.device_count()) / (time.perf_counter() - epoch_start_time),
#                             })
...
#             eval_ppl, eval_epoch_loss, temp_val_loss, temp_step_perplexity = evaluation(model, train_config, eval_dataloader, local_rank, tokenizer, wandb_run, epoch=epoch)
#             if train_config.save_metrics:
...
def train(model, train_dataloader, eval_dataloader, tokenizer, optimizer, lr_scheduler, gradient_accumulation_steps, train_config, fsdp_config=None, local_rank=None, rank=None, wandb_run=None):
        ...
        with MemoryTrace() as memtrace:  # track the memory usage
            ...
            with profile(train_config, local_rank) as profile_context:
                for step, batch in enumerate(train_dataloader):
                    total_steps += 1
                    # stop when the maximum number of training steps is reached
                    if train_config.max_train_step > 0 and total_steps > train_config.max_train_step:
                        max_steps_reached = True
                        if not train_config.enable_fsdp or local_rank == 0:
                            print("max training steps reached, stopping training, total train steps finished: ", total_steps-1)
                        break
                    ...
                    if total_steps % 5 == 0:
                        ...
                        eval_ppl, eval_step_loss, temp_val_loss, temp_step_perplexity = evaluation(
                            model, train_config, eval_dataloader, local_rank, tokenizer, wandb_run, step=total_steps
                        )
                        if train_config.save_metrics:
                            val_step_loss.extend(temp_val_loss)
                            val_step_perplexity.extend(temp_step_perplexity)
                        
                        if eval_step_loss < best_eval_loss:
                            best_eval_loss = eval_step_loss
                            ...
                            if train_config.save_model:
                                ...
                                if train_config.use_peft:
                                    if train_config.enable_fsdp:
                                        if rank == 0:
                                            print(f"Found a better hold out loss! Saving PEFT modules at step {total_steps}")
                                    else:
                                        print(f"Found a better hold out loss! Saving PEFT modules at step {total_steps}")
                                    model.save_pretrained(f"{train_config.output_dir}/best_model_step_{total_steps}")
                                else:
                                    if fsdp_config.checkpoint_type == StateDictType.FULL_STATE_DICT:
                                        save_model_checkpoint(model, optimizer, rank, train_config, step=total_steps)
                                    elif fsdp_config.checkpoint_type == StateDictType.SHARDED_STATE_DICT:
                                        print(f"Saving FSDP model checkpoints using SHARDED_STATE_DICT at step {total_steps}")
                                        save_model_and_optimizer_sharded(model, rank, train_config)
                                    
                                    if train_config.save_optimizer:
                                        save_optimizer_checkpoint(model, optimizer, rank, train_config, step=total_steps)
                            ...
                            if train_config.enable_fsdp:
                                if rank == 0:
                                    print(f"Best loss on holdout  {best_eval_loss:.4f} at step {best_step}")
                            else:
                                print(f"Best loss on holdout {best_eval_loss:.4f} at step {best_step}")
                        
                        val_loss.append(float(best_eval_loss))
                        val_prep.append(float(eval_ppl))
    ...
    results["avg_epoch_time"] = avg_epoch_time
    results["avg_checkpoint_time"] = avg_checkpoint_time
    results["best_eval_loss"] = best_eval_loss
...
def evaluation(model,train_config, eval_dataloader, local_rank, tokenizer, wandb_run, step=None):
    """
    ...
    with MemoryTrace() as memtrace:
        for eval_step, batch in enumerate(tqdm(eval_dataloader,colour="green", desc="evaluating Epoch", dynamic_ncols=True)):
            total_eval_steps += 1
    ...
    if wandb_run:
      ...
      print(f"highest F1 score at between steps {step} and {step - train_config.logging_steps + 1} is {best_f1_score}, with threshold {best_threshold}")


      wandb_run.log({
          'eval/best_f1': best_f1_score,
...
# def evaluation(model,train_config, eval_dataloader, local_rank, tokenizer, wandb_run, epoch=None):
#     """
#     Evaluates the model on the given dataloader
